<template lang="pug">
  div.full-width.flex-container.flex-column.full-height
    h3 Conclusions
    .conclusions
        ...
        div(v-if='step <= 4')
          ...
          ul
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 2') The moment when the pedestrian or cyclist appears was shown to be crucial
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 3') Similar brake onset behaviour in simulator and test-track environments
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 4') Similar self-reported discomfort in simulator and test-track environments
      eg-transition(enter='slideInRight' leave='slideOutLeft')
        div(v-if='step >= 5 && step <=8')
          DriverModelSVG.itemLogo
          p.itemText Devise driver models
          ul
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 6') Models for key points of the driver response
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 7') Model for discomfort model
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 8') Model predicting driver brake pedal deflection
      eg-transition(enter='slideInRight' leave='slideOutLeft')
        div(v-if='step >= 9 && step <=11')
          ...
          ul
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 10') Results in line with Euro NCAP test protocols
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 11') FCW systems should be considered by the assessment programmes
      eg-transition(enter='slideInRight' leave='slideOutLeft')
        div(v-if='step >= 12')
          ...
          ul
            eg-transition(enter='fadeIn' leave='fadeOut')
              li(v-if='step >= 13') Safety benefit evaluation highly-depends on the driver model
</template>
<script>
